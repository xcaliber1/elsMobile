  ...
  import { collection, getDoc, addDoc, doc, query, where, getDocs, updateDoc } from 'firebase/firestore'; 
...
const [editProfileData, setEditProfileData] = useState({
  ...
  profileImageUri: userData.profileImageUri,
    ...
    // Function to initiate a call to the specified phone number
const handlePhoneCall = () => {
  const phoneNumber = '09992277373';
...
const openEditProfile = () => {
  setEditProfileData(userData); // Set initial values to the current user data
...
const saveProfileChanges = async () => {
  try {
    ...
    const q = query(collection(firestore, 'personalInfo'), where('email', '==', userData.email));
    ...
    if (!querySnapshot.empty) {
      ...
      await updateDoc(userDoc.ref, {
        ...
        profileImageUri: editProfileData.profileImageUri,
      });
...
const handleEditProfileImageUpload = async () => {
  ...
  if (!result.canceled) {
    setEditProfileData({ ...editProfileData, profileImageUri: result.assets[0].uri });
  }
};
    ...
    const sendEmergencyData = async () => {
      try {
        ...
        // Include profile image in emergency data
        let profileImageDownloadUrl = '';
        if (userData.profileImageUri) {
          const response = await fetch(userData.profileImageUri);
          const blob = await response.blob();
          const profileImageRef = storageRef(storage, `profileImages/${Date.now()}`);
          await uploadBytes(profileImageRef, blob);
          profileImageDownloadUrl = await getDownloadURL(profileImageRef);
        }
        ...
        await set(newEmergencyRef, {
          ...
          // Include personal information and profile image
          personalInfo: {
            firstname: userData.firstname,
            lastname: userData.lastname,
            phoneNumber: userData.phoneNumber,
            email: userData.email,
            profileImageUri: profileImageDownloadUrl, // Add profile image URL here
          },
        });
    
        // Send emergency email to response team via Laravel backend
        await axios.post('http://192.168.243.71:8000/api/report-emergency', {
          emergencyType: selectedEmergency,
          description,
          location: {
            latitude: locationResult.coords.latitude,
    ...
    const handleEmailSubmit = async () => {
      ...
      try {
        ...
        const response = await axios.post('http://192.168.243.71:8000/api/send-verification-code', {
    ...
    const handleCodeSubmit = async () => {
      try {
        const response = await axios.post('http://192.168.243.71:8000/api/verify-code', {
          email: personalInfo.email,
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity style={styles.footerButton} onPress={handlePhoneCall}>
    ...
    >
      <View style={styles.editProfileContainer}>
        <View style={styles.editProfileModal}>
          ...
          <TouchableOpacity style={styles.uploadButton} onPress={handleEditProfileImageUpload}>
            <Text style={styles.uploadButtonText}>Change Profile Image</Text>
          </TouchableOpacity>
          {editProfileData.profileImageUri ? (
            <Image source={{ uri: editProfileData.profileImageUri }} style={styles.profileImagePreview} />
          ) : null}
          ...
          <TextInput
            ...
            onChangeText={(text) => setEditProfileData({ ...editProfileData, firstname: text })}
          />
          ...
          <TextInput
            ...
            onChangeText={(text) => setEditProfileData({ ...editProfileData, lastname: text })}
          />
          ...
          <TextInput
            ...
            onChangeText={(text) => setEditProfileData({ ...editProfileData, phoneNumber: text })}
          />

          {/* Save Changes Button */}
          <TouchableOpacity style={styles.submitButton} onPress={saveProfileChanges}>
